...
export default class App extends React.Component {
  ...
  selectSequence = () => {
    ...
    if (this.state.time === 0) {
      ...
      if (
        this.state.time ===
        this.state.sequenceStartTime - this.state.sequenceTime
      ) {
        this.setState({
  ...
  render() {
    ...
    if (paused) {
      button = (
        <Button color="red" title="start" onPress={this.handlePlayTimer} />
      );
    } else {
      button = (
        <Button
    ...
    return (
      <View style={styles.container}>
        ...
        <Button color="white" title="reset" onPress={this.handleCancelTimer} />
        <SequenceTimer sequenceDisplay={sequenceDisplay} />
        {button}
